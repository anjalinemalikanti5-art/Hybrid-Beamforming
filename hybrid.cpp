...
int main() {
    ...
    for (int i = 0; i < N; i++) theta_ml(i) = alpha + beta * X_index(i);

    ofstream ml_out("ml_doa_pred.csv");
    for (int i = 0; i < N; i++) ml_out << i << "," << theta_ml(i) << endl;
    ml_out.close();
    ...
    /* ================= BEAMFORMERS ================= */
    VectorXcd a_s = steering_vector(theta_ml.mean(), M); // ML DOA used here
    ...
    for (double alpha = 0.0; alpha <= 1.0; alpha += 0.01) {
        ...
        if (snr > best_snr) { best_snr = snr; best_alpha = alpha; }
    }
    ...
    double snr_ml = 10 * log10(y_ml_sig.cwiseAbs2().mean() / max(EPS, (y_ml - y_ml_sig).cwiseAbs2().mean()));

    RowVectorXcd y_mv = w_mvdr.adjoint() * X;
    RowVectorXcd y_mv_sig = w_mvdr.adjoint() * X_sig;
    double snr_mvdr = 10 * log10(y_mv_sig.cwiseAbs2().mean() / max(EPS, (y_mv - y_mv_sig).cwiseAbs2().mean()));

    RowVectorXcd y_hyb = w_hybrid.adjoint() * X;
    RowVectorXcd y_hyb_sig = w_hybrid.adjoint() * X_sig;
    double snr_hybrid = 10 * log10(y_hyb_sig.cwiseAbs2().mean() / max(EPS, (y_hyb - y_hyb_sig).cwiseAbs2().mean()));
    ...
    /* ================= SAVE DATA FOR PLOTS ================= */
    ofstream ml_beam("beam_ml.dat"), mvdr_beam("beam_mvdr.dat"), hyb_beam("beam_hybrid.dat");
    ofstream bef("before.dat"), aft("after.dat");
    ofstream doa_plot("doa_estimation.dat"), snr_alpha("snr_vs_alpha.dat");
    ofstream beam_all("beam_all.dat"), signals_comp("signals_comparison.dat");

    for (int ang = -90; ang <= 90; ang++) {
        VectorXcd sv = steering_vector(ang, M);
        ml_beam << ang << " " << 20*log10(abs((w_ml.adjoint()*sv)(0,0)) + EPS) << endl;
        mvdr_beam << ang << " " << 20*log10(abs((w_mvdr.adjoint()*sv)(0,0)) + EPS) << endl;
        hyb_beam << ang << " " << 20*log10(abs((w_hybrid.adjoint()*sv)(0,0)) + EPS) << endl;

        beam_all << ang << " "
                 << 20*log10(abs((w_ml.adjoint()*sv)(0,0)) + EPS) << " "
                 << 20*log10(abs((w_mvdr.adjoint()*sv)(0,0)) + EPS) << " "
                 << 20*log10(abs((w_hybrid.adjoint()*sv)(0,0)) + EPS) << endl;
    }

    for (int n = 0; n < N; n++) {
        bef << n << " " << real(X(0,n)) << endl;
        aft << n << " " << real(y_hyb(n)) << endl;

        doa_plot << n << " " << theta_s_noisy(n) << " " << theta_s_hat(n) << " " << theta_ml(n) << endl;

        double alpha_tmp = double(n) / double(N-1);
        VectorXcd w_tmp = (alpha_tmp*w_ml + (1-alpha_tmp)*w_mvdr).normalized();
        RowVectorXcd y_tmp = w_tmp.adjoint() * X;
        RowVectorXcd y_sig = w_tmp.adjoint() * X_sig;
        RowVectorXcd y_n = y_tmp - y_sig;
        double Ps = y_sig.cwiseAbs2().mean();
        double Pn = max(EPS, y_n.cwiseAbs2().mean());
        double snr_val = 10 * log10(Ps / Pn);
        snr_alpha << alpha_tmp << " " << snr_val << endl;

        RowVectorXcd y_ml_row = w_ml.adjoint() * X.col(n);
        RowVectorXcd y_mv_row = w_mvdr.adjoint() * X.col(n);
        RowVectorXcd y_hyb_row = w_hybrid.adjoint() * X.col(n);
        signals_comp << n << " " << real(X(0,n)) << " "
                     << real(y_ml_row(0)) << " "
                     << real(y_mv_row(0)) << " "
                     << real(y_hyb_row(0)) << endl;
    }

    ml_beam.close(); mvdr_beam.close(); hyb_beam.close();
    bef.close(); aft.close();
    doa_plot.close(); snr_alpha.close();
    beam_all.close(); signals_comp.close();

    /* ================= 3D HYBRID BEAM PATTERN ================= */
    ofstream bp_hybrid("beampattern_hybrid_3d.dat");
    for (int n = 0; n < N; n++) {
        for (int ang = -90; ang <= 90; ang++) {
            VectorXcd sv = steering_vector(ang, M);
            bp_hybrid << n+1 << " " << ang << " "
    ...
    bp_hybrid.close();
// ================= SNR COMPARISON BAR =================
ofstream snr_bar("snr_comparison.dat");
...
snr_bar.close();
    ...
    system("gnuplot -persist -e \"set title 'Signal Comparison'; set xlabel 'Snapshot'; set ylabel 'Amplitude'; set grid; plot 'signals_comparison.dat' using 1:2 with lines title 'Input', 'signals_comparison.dat' using 1:3 with lines title 'ML Output', 'signals_comparison.dat' using 1:4 with lines title 'MVDR Output', 'signals_comparison.dat' using 1:5 with lines title 'Hybrid Output'\"");
    system("gnuplot -persist -e \"set title 'Beam Patterns (ML/MVDR/Hybrid)'; set xlabel 'Angle (deg)'; set ylabel 'Magnitude (dB)'; set grid; plot 'beam_all.dat' using 1:2 with lines title 'ML', 'beam_all.dat' using 1:3 with lines title 'MVDR', 'beam_all.dat' using 1:4 with lines title 'Hybrid'\"");
    ...
    system("gnuplot -persist -e \"set title 'SNR Comparison'; set style data histograms; set style fill solid 1.0 border -1; set ylabel 'SNR (dB)'; set grid ytics; plot 'snr_vs_alpha.dat' using 2:xtic(1) lc rgb 'skyblue'\"");
    // Use correct SNR values for plotting
system(
    "gnuplot -persist -e \""
    "set title 'SNR Comparison';"
    "set style data histograms; set style fill solid 1.0 border -1;"
    "set ylabel 'SNR (dB)'; set grid ytics;"
    "plot 'snr_comparison.dat' using 2:xtic(1) lc rgb 'skyblue'\""
);
